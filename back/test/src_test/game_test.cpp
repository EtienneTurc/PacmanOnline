...
TEST_F(GameTest, getPacmans_addPacman) {
	...
	EXPECT_EQ(1, pacmans[0].getXPosition());
	EXPECT_EQ(4, pacmans[0].getYPosition());
	EXPECT_EQ(DOWN, pacmans[0].getDirection());
	EXPECT_EQ(1, pacmans[0].getSpeed());
}
...
TEST_F(GameTest, getGhosts_addGhost) {
	...
	EXPECT_EQ(1, ghosts[0].getXPosition());
	EXPECT_EQ(4, ghosts[0].getYPosition());
	EXPECT_EQ(DOWN, ghosts[0].getDirection());
	EXPECT_EQ(1, ghosts[0].getSpeed());
}
